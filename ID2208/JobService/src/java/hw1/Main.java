    ...
    public static void process(String profile, String trans, String record, String info, String cv) {
		...
		try {
			...
			List<hw1.jaxb.employmentRecord.CompanyType> companies = ((JAXBElement<hw1.jaxb.employmentRecord.RecordType>) jc
					.createUnmarshaller().unmarshal(
							new File(record))).getValue().getCompanies().getCompany();
...
//					"CompanyInfo.xsd");
			...
			// input XML file.
			// /
			Document document = builder.parse(new File(info));
		...
		try {
	        ...
	        m.marshal(xml, new BufferedWriter(new FileWriter(profile)));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void main(String[] args) {
		process("./xml/ApplicantProfile.xml", "./xml/Transcripts.xml", "./xml/EmploymentRecord.xml",
                "./xml/CompanyInfo.xml", "./xml/ShortCV.xml");
