...
public class Application1b extends ComponentDefinition {
	Positive<EpfdLink> epfd = positive(EpfdLink.class);
	...
	public Application1b() {
		...
		subscribe(handlePp2pMessage, epfd);
	}
	...
	Handler<Restore> handleRestore = new Handler<Restore>() {
		public void handle(Restore event) {
			logger.info("Node {} was restored! Period={}", event.getNode().getId(), event.getPeriod());
		}
	};
	
	Handler<Suspect> handleSuspect = new Handler<Suspect>() {
		public void handle(Suspect event) {
			logger.info("Node {} was suspected! Period={}", event.getNode().getId(), event.getPeriod());
		}
	};
	...
	private final void doPerfect(String message) {
		for (Address neighbor : neighborSet) {
			...
			trigger(new Pp2pSend(neighbor, new Pp2pMessage(self, message)), epfd);
		}
	}
